...
標題可以稍微整理使用者的口語問法，但要保留原本技術意圖。標題用直述句或疑問句都可以，看哪種讀起來更自然。若使用者用語音輸入造成小錯字或轉錄偏差，直接改成正確術語，不需要特別指出。

## 內容與風格

以短段落為主，常搭配少量 bullet、短指令區塊或簡短對照。

實際上最常見、也最推薦的結構是「起承轉合」：

1. 起：先用 1 句短答直接回答
2. 承：中間用 2 到 4 個 bullet 拆關鍵理由、層次或對照
3. 轉：當題目涉及取捨、使用情境或常見誤解時，用 1 到 2 個短段落整合重點；若題目單純，可省略
4. 合：最後用 1 句「一句話記法」收斂

如果題目非常小，也可以只有短段落加一句話記法，不一定每題都硬加 bullet。如果題目已經能在「起、承、合」內講清楚，就不要硬補「轉」。

風格範例請參照 `review/note-samples.md`，寫入時以該檔案的簡潔程度為標準。

例外：如果有重要的、可以一目了然的表格，應該保留。表格適合用於對照、比較、取捨類的內容。

表格、ordered list、code block 都可以使用，只要它們能明顯提升可讀性。

- 表格：適合對照、比較、取捨
- ordered list：只用在明顯有順序、固定步驟或先後流程的情況
- 一般列點：若沒有強烈順序需求，預設使用 bullet list 即可

每一題的內容應該：

- 先直接回答問題
- 補上必要原因、邊界或 trade-off
- 當問題明確涉及 WeaMind 或需要 repo 依據時，使用 WeaMind repo 內的實際架構、manifest、文件或 lesson note 作為依據
- 寫成使用者之後能直接拿來重講的版本
- 避免把一題擴成完整 lesson
...
- 若重點超過 4 個 bullet，優先拆成短段落、表格或更小的對照，不要把所有資訊硬塞進同一串列表
