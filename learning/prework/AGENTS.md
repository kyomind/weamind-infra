...
每份 prework 通常會在同一份文件中同時放兩件事：前半段是要貼給外部 AI 的教學任務，後半段是學完後要帶回 VS Code 的學習報告回填區。前者負責建立理解骨架，後者負責把結果收斂成可保存的紀錄。
...
4. 若直接進 lesson，Copilot 很可能先花很多段在講通用知識，還無法立刻進入 YAML、文件或 debug sequence。
