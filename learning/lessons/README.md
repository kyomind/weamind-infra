...
若今天的 lesson 明確屬於 command-heavy workshop，而不是概念展開型 lesson，QA 可縮成 2 到 3 題短題，但仍應保留。這類 QA 的角色不是重講大塊概念，而是先對齊：
...
5. 在 `### 答題引導` 區塊裡，若題目本身有明確順序，使用 ordered list；若只是提醒要回答哪些重點，使用一般 bullet list。
6. 在 `### AI 修正與補充` 這個區塊裡，AI 應只把新結論中的少數「關鍵句」或「關鍵詞」加粗，方便之後複習時快速掃到重點；不要把整段結論或連續多句話大面積加粗。
7. 若 AI 回答中出現 `kubectl` 指令、Pod 狀態、欄位名、資源名、環境變數名或其他技術術語，應優先使用 inline code 標示，例如 `events`、`CrashLoopBackOff`、`POSTGRES_HOST`、`kubectl logs --previous`，讓自然語言與術語更容易分開掃讀。
...
3. 這裡所說的「先建好題目骨架」，預設是先寫出今天預計的 2 到 4 輪情境、每輪要驗證的問題，以及候選指令區塊；若今天明確是 command-heavy workshop，則固定以 5 到 6 輪微情境規劃，但每輪仍應維持單一判讀閉環。實際的使用者選擇、輸出、AI 判讀與收斂，仍在互動過程中逐輪回填。
...
4. 若今天是 command-heavy workshop，與其把每一輪做成又大又慢的綜合題，更好的做法通常是把單位切小，改成 5 到 6 輪微情境；增加的是判讀次數，不是單輪複雜度。
5. command-heavy workshop 的多輪題，建議分成 2 個小段進行，例如 3 輪加 3 輪，避免單次互動過長、後段疲勞導致品質下降。
6. 每一輪若需要保留當時的情境、三個選項或 AI 給的最小提示，應直接放進該輪 command 區塊內，不另外插一層獨立的互動題區塊。
7. `03-command.md` 裡的指令與輸出都統一使用 `bash` code block，不再混用 `text` 或其他標記。
8. `03-command.md` 的首要目標是降低複習阻力，因此只保留關鍵輸出與必要上下文；若原始輸出很長，預設只摘錄最有判讀價值的 3 到 8 行。
9. 若某輪真正有價值的是「原本怎麼誤判、後來怎麼修正」，可以在該輪補一小段「常見誤解 / 這輪釐清了什麼」，但不要把它寫成第二份 QA。
10. 若 AI 為了驗證環境而代跑指令，應明確標示那只是輔助驗證，不等於使用者已完成 command drill。
11. command drill 的最小完成條件，預設是使用者至少親手完成一輪最小操作、貼回輸出，並說出自己的選擇理由。
12. 在 `### AI 判讀與修正` 這個區塊裡，AI 應主動把「關鍵句」、「關鍵詞」或「判讀口訣」加粗，讓之後回看時能先掃到最有價值的判讀；但同樣不要把整段全面加粗。
13. 若 AI 判讀中出現 `kubectl` 指令、狀態名、事件欄位、資源欄位、container / probe / env key 等技術術語，也應優先使用 inline code 標示，避免和一般敘述混在一起。
