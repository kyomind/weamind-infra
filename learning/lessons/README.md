...
3. 每題預設都要有一個 `### 拆題` 區塊，作為回答引導。
3. 避免大量單行粗體欄位，讓整份 QA 更穩定、好掃讀。
4. 題目應盡量小、具體、可直接對照 repo，不要把一題寫成一整個章節。
5. 在 `### 拆題` 區塊裡，若題目本身有明確順序，使用 ordered list；若只是提醒要回答哪些重點，使用一般 bullet list。
...
2. 先給完整主問題，再搭配 `### 拆題` 提供回答引導，讓使用者一開始就知道這題應回答哪些面向或應依什麼順序回答。
3. 這個引導不是為了暴露答案，而是為了降低答題邊界的不確定性；引導應聚焦在回答面向、順序或比較軸線，不要直接把結論寫出來。
4. 回答引導是 QA 的預設結構，不需要等使用者說「太難了」才補上。
...
1. QA 題目預設就應搭配 `### 拆題`，不再以「使用者卡住後才拆題」為前提。
2. `### 拆題` 的功能是提供回答引導，不是把 QA 改寫成選擇題。
3. QA 預設不使用選擇題；選擇題式三選一保留給 `03-command.md` 的 command drill。
4. AI 應將大題拆成 2 到 4 個子題或回答面向，不另開新 Q 編號。
...
8. 建立 lesson 骨架時，預設就應為每一題先寫出最小的 `### 拆題` 引導，不再等互動過程中才補。
...
3. 這裡所說的「先建好題目骨架」，是指先寫出今天預計的 2 到 4 輪情境、每輪要驗證的問題，以及候選指令區塊；實際的使用者選擇、輸出、AI 判讀與收斂，仍在互動過程中逐輪回填。
...
4. 每一輪若需要保留當時的情境、三個選項或 AI 給的最小提示，應直接放進該輪 command 區塊內，不另外插一層獨立的互動題區塊。
5. `03-command.md` 裡的指令與輸出都統一使用 `bash` code block，不再混用 `text` 或其他標記。
6. `03-command.md` 的首要目標是降低複習阻力，因此只保留關鍵輸出與必要上下文；若原始輸出很長，預設只摘錄最有判讀價值的 3 到 8 行。
7. 若某輪真正有價值的是「原本怎麼誤判、後來怎麼修正」，可以在該輪補一小段「常見誤解 / 這輪釐清了什麼」，但不要把它寫成第二份 QA。
8. 若 AI 為了驗證環境而代跑指令，應明確標示那只是輔助驗證，不等於使用者已完成 command drill。
8. command drill 的最小完成條件，預設是使用者至少親手完成一輪最小操作、貼回輸出，並說出自己的選擇理由。
9. 在 `### AI 判讀與修正` 這個區塊裡，AI 應主動把「關鍵句」、「關鍵詞」或「判讀口訣」加粗，讓之後回看時能先掃到最有價值的判讀；但同樣不要把整段全面加粗。
