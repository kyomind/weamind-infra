...
5. `Notes` 內的 H3 使用語意標題，不需要加 `1.`、`2.` 這類編號；新版以不編號為準。
6. `Notes` 的單則內容格式可以彈性安排。預設偏好是先用一小段前言交代問題來源或背景，再用 bullet list 展開要點，最後用單獨段落收斂成一句話或一個最小結論；若內容很短，也可以只用前言加收斂句，不必硬湊條列。
7. 若 note 需要放指令、輸出片段或明確步驟，可以突破前述格式，改用 code block、ordered list 或更適合閱讀的段落安排；重點是讓之後複習時能快速抓到問題、推理和結論。
8. `學習注意事項` 用來放外部預習回帶重點、lesson 邊界、待驗證的 repo 對照點、今天暫不展開的點。
9. 初始化 `05-note.md` 時，只有 `學習注意事項` 可以先回填外部預習內容；`Notes` 與 `Flashcards` 兩個 H2 區塊一開始必須留空，等 QA、command 或 lesson 過程中真的出現延伸問答、暫時結論與卡片素材後，再往裡面補。
10. 初始化時，`Notes` 與 `Flashcards` 可以只保留模板中的 HTML comment 這類特殊註記作為佔位；這種註記只是提示「這裡之後會長內容」，不算真正已回填內容。
11. `Notes` 用來放 lesson 過程中的延伸提問、補充解釋與尚未進 `04-report.md` 的暫時結論；不要在初始化時就先把外部學習摘要、骨架提示或預設結論放進去。
12. 若某段內容原本來自 QA 中途的額外追問，而且它不影響當前主題的最小驗收，預設應把詳細展開放進 `Notes`，不要把 `02-qa.md` 撐成過長的追問紀錄。
13. 若某段內容原本來自 command drill 中途的額外追問，而且它不影響當輪操作的最小閉環，預設也應把詳細展開放進 `Notes`，不要把 `03-command.md` 撐成第二份 QA 或第二份 note。
14. `Flashcards` 固定保留在檔案內，但初始化時必須留空；除了模板中的佔位註記外，不應先填入真正卡片。只有在 lesson 過程中真的整理出卡片後，才開始填寫。
15. `Flashcards` 的詳細生成規則、格式細節與精修原則，交由 `.github/prompts/generate-flashcards.prompt.md` 維護；本檔只保留 lesson 結構本身需要知道的最小規則。
