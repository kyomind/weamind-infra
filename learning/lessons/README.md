...
5. 在 `### AI 修正與補充` 這個區塊裡，AI 應主動把關鍵句、核心結論或高價值關鍵詞加粗，方便之後複習時快速掃到重點；但不要把整段幾乎每一句都加粗，避免失去層次。
...
9. 在 `### AI 判讀與修正` 這個區塊裡，AI 應主動把關鍵句、核心結論或判讀口訣加粗，讓之後回看時能先掃到最有價值的判讀；但同樣不要把整段全面加粗。
...
5. 第二行起為縮排子項目，作為卡片背面。
6. **背面最多五行，視觀念需要調整**，重點放最小必要上下文，讓單張卡片獨立看也能理解大意。
7. 優先保留三個月後仍有價值的高含金量內容，不要把所有糾正細節都做成卡片。
8. 卡片預設整理在 `05-note.md`，不需要另外開一份專屬卡片檔。
9. `Flashcards` 區塊內直接放卡片條列，不另外用 H3 分組卡片批次；若需要表達批次，放在對話或 commit 脈絡中即可，不寫進卡片結構本身。
