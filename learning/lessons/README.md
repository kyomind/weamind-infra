...
5. 在 `### AI 修正與補充` 這個區塊裡，AI 應主動把「關鍵句」或「關鍵詞」加粗，方便之後複習時快速掃到重點；但不要把整段幾乎每一句都加粗，避免失去層次。
...
7. 若 lesson 停在 `02-qa.md`，而使用者只說「continue」或「繼續」，預設是回到當前 QA 互動回合：繼續下一個主問題、同一題的下一個引導點，或同一題的收斂，不是直接代寫後面所有題目。
