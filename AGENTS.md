...
- 若使用者在 QA 過程中提出額外追問，AI 應先判斷它是否仍屬於當前主問題的最小完成範圍；若該追問獨立性較高、偏延伸補充，或放進 QA 會讓複習阻力明顯上升，應優先整理到 `05-note.md`，只在 `02-qa.md` 保留完成當題所需的最小收斂結論。
- 若使用者在 command drill 過程中提出額外追問，AI 也應先判斷它是否仍屬於當輪操作的最小閉環；若該追問獨立性較高、偏延伸補充，或放進 `03-command.md` 會破壞這一輪的可複習性，應優先整理到 `05-note.md`，只在 `03-command.md` 保留完成當輪判讀所需的最小結論。
