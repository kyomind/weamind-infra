...
implement-heavy mode 預設直接建立七份檔案：
...
1. 先用 `06-implementation.md` 做主要實作與每輪閉環記錄。
2. 若 `06` 過程中出現 implementation-specific 補充觀察，同步整理到 `07-implementation-note.md`。
3. 實作主體結束後，再回 `02-qa.md` 做實作後定位與驗證收斂。
4. 若需要最小操作驗證，再用 `03-command.md` 補實作前後證據。
...
3. `03-command.md`：承接實作前後的最小操作驗證與證據。
4. `04-report.md`：收斂今天真正學到的內容。
5. `05-note.md`：記錄一般 lesson 延伸問答、暫時結論與卡片整理。
6. `06-implementation.md`：記錄今天的主要實作閉環。
7. `07-implementation-note.md`：只承接 `06` 過程中的 implementation-specific 補充觀察。
...
> 這份檔案與 `06-implementation.md` 綁定，只承接 `06` 過程中的 implementation-specific 補充觀察。
