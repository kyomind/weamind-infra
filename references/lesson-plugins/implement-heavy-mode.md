...
8. 若某個 step 目前完成的只是「設計稿、策略稿、query 草案、版面規劃」而非實際落地，`實際執行內容與結果` 必須明講它仍是設計層成果，不能寫成已完成 runtime 驗證。
9. 在這種設計先行的 step 裡，`AI 判讀與收斂` 只能收斂到「這份設計是否已足夠進入下一步實作」，不能提前寫成「功能已驗證成功」或「畫面已完成」。
10. `目前狀態` 應如實區分，例如可寫成「已完成（設計稿）」或「進行中（待實作驗證）」，避免把設計完成誤寫成整步已執行完成。
