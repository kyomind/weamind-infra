...
2. `預計採取的動作` 應維持高層次，交代這一步打算做哪些事。
3. `實際執行內容` 可作為操作稿或執行稿，寫給使用者照著做也可以；這裡適合放較具體的指令、查詢、panel 規格、操作順序或檢查點。
4. `結果` 才用來記真的做完之後看到的輸出、畫面、數值、現象與關鍵觀察；不要把尚未發生的結果預寫在這裡。
5. 不需要為不同型態再拆多套 step 模板；維持同一套骨架，但讓內容跟著這一步的實際型態走。
6. 不要求每一輪對話、每一次中間試錯、或每一個局部猜測都立刻同步回寫到 `06-implementation.md`；較長的 step 可以先持續協作與討論，等到出現明確進展、形成一段可複習的證據鏈，或 step 正式收斂時，再擇要回填。
7. `06-implementation.md` 應優先記錄對之後複習真的有價值的內容，例如主要決策、關鍵操作、代表性證據、結果與收斂，而不是完整保留所有中間來回、試錯細節或每輪對話痕跡。
8. 若某個 step 討論時間較長，AI 應先把互動留在對話裡推進，不必因為每次小變化就反覆改檔；等到這一步邊界比較清楚時，再集中整理進該 step。
9. 若某個 step 目前完成的只是「設計稿、策略稿、query 草案、版面規劃」而非實際落地，`實際執行內容` 可以完整保留這份設計稿，但 `結果` 必須明講尚未做 runtime 驗證。
10. 在這種設計先行的 step 裡，`AI 判讀與收斂` 只能收斂到「這份設計是否已足夠進入下一步實作」，不能提前寫成「功能已驗證成功」或「畫面已完成」。
11. `目前狀態` 應如實區分，例如可寫成「已完成（設計稿）」或「進行中（待實作驗證）」，避免把設計完成誤寫成整步已執行完成。
...
1. 可以先寫驗證目標與預計採取的動作。
2. `實際執行內容` 可以先寫操作稿或設計稿，但 `結果`、`AI 判讀與收斂`、`目前狀態` 在還沒真的動手前，不應預寫成已完成結果。
