...
5. 建立完今天要用的文件後，正式開始課程前，應再做一次文件自檢，重新對照本檔與需要時的 `lesson-template.md`，確認順序、骨架與段落沒有明顯錯誤。
...
若已判斷今天需要外部預習，預設應先完成 outline 與外部學習，再建立或進入 lesson；不要先把 lesson 問答、command 骨架一併展開。

若今天已建立 lesson，正式開始前建議至少檢查以下幾件事：

1. 內部流程是否仍是 QA → command → report。
2. `03-command.md` 若存在，是否真的符合 command drill 的節奏，而不是單純指令清單。
3. `04-report.md` 是否有對應當天實際存在的 command drill。
...
5. QA 因為範圍通常比 command drill 大，所以可以保留一定程度的引導，幫助使用者先抓到題目的邊界與回答方向。
6. 但 QA 的引導仍應節制，不要在主問題剛拋出時就把答案路徑鋪好；原則是先讓使用者自行組織答案，只有在明確卡住時才逐步加提示。
...
5. command drill 本身已經透過情境與三選一提供了基本引導，因此 AI 不應再額外加上「建議先走哪條最省力路線」或過度縮小判斷空間。
6. 若使用者先選了不夠精準但仍合理的指令，應優先保留這個試錯與修正過程，因為這本身就是 command drill 的一部分。
...
9. command drill 的收斂重點是判讀與修正，不是預先替使用者排除所有錯誤選項；若過度提示，會削弱這一輪的辨識價值。
...
若今天有外部預習，則完整順序應是：先完成 `docs/outlines/` 的外部預習，再進入上面這套 lesson 流程。
