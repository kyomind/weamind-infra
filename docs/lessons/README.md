...
2. 較好的節奏是：AI 先出最小觀察題，再附必要的指令提示或選擇題；使用者自行執行後回報輸出；AI 再協助判讀與收斂。
3. 若 AI 為了驗證環境而代跑指令，應明確標示那只是輔助驗證，不等於使用者已完成 command drill。
...
6. 若 lesson 結尾需要整理 Logseq 閃卡，可直接在 `05-note.md` 內用獨立的 H2 區塊集中放卡片。

建議做法：

1. lesson 進行中先以學習與收斂為主，不急著即時輸出大量卡片。
2. 等 session 結尾，再把真正值得留下的高含金量重點整理進 `05-note.md`。
3. 卡片區塊應只放最後保留的內容，不要把所有候選草稿都堆進去。
4. 若當下只需要少量即時卡片，也可先輸出到對話中；但正式收斂版本仍以 `05-note.md` 為主。
...
8. 可適度使用 `**粗體**` 與 `code` 提高可讀性。

範例：(僅供參考，背面內容可以1到5行不等)
...
1. command drill 的核心是讓使用者建立操作與判讀能力，因此預設應由使用者自己執行至少一輪最小指令，再由 AI 協助判讀。
2. 若當下只能由 AI 先代跑驗證，也應在 lesson 中明確標示那只是輔助觀察，不可直接當成使用者已完成練習。
...
1. 卡片可放在 `05-note.md`，不需要再另外開一份專屬卡片檔。
2. 卡片區塊應視為 note 的一部分，建議用獨立 H2 標示，例如 `## Flashcards`。
3. `05-note.md` 放的是卡片的正式整理版，不是所有中途候選片段。
4. 若未特別指定，AI 產卡時預設遵循本檔的 Logseq 閃卡格式。
