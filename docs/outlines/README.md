...
1. 這一天是否需要外部純知識預習。
2. 如果需要，先建立當天的 outline，再去外部 AI 預習。
3. 如果不需要，直接進入 VS Code 內的 lesson 流程。

一句話原則：先判斷要不要外部預習，再決定建立 outline 或 lesson。
...
4. 一天原則上一份 outline；若真的拆成兩份，第二份應在 slug 上明確區分。
...
所以每份 outline 的核心目的，是把外部 AI 的教學任務、範圍與節奏寫清楚，讓它自然聚焦在「純知識預習」與「最小理解骨架」，而不是自行延伸到專案細節。

---

## 何時需要新增一份 outline

遇到下面情況，就應新增當天 outline：
...
那就不一定需要 outline，可以直接在 VS Code 內進行。
...
2. 先判斷今天是否真的需要外部預習；只有答案是「需要」時，才建立 outline。
3. 判斷今天有哪些內容屬於「純知識預習」，哪些內容應保留到 VS Code 內做專案對照。
4. 在 `docs/outlines/` 建立新檔，檔名依 `YYYY-MM-DD-slug.md`。
5. 內容必須明確寫出：
   - 今天要學什麼
   - 這份 outline 要怎麼用
   - 今天要建立的最小理解骨架
   - 建議教學順序
   - 學完後要產出的學習報告格式
6. 寫完後，若有必要，更新 `.privatedocs/ai-memories.md`，記錄新的 outline 已建立。
...
outline 的目標不是讓外部 AI 一開始就考使用者，而是先幫使用者聽懂今天最需要的基礎。

### 原則 1.5：保持輕量

outline 應盡量短、小、直接。目標是讓外部預習控制在 45 到 60 分鐘左右，而不是把 outline 寫成一份需要花 2 小時執行的微型課程。
...
outline 應優先寫清楚今天的教學任務、範圍、節奏與輸出。當責任被定義得夠清楚時，外部 AI 自然較容易維持聚焦，不需要堆疊太多負向限制語句。
...
之後新增 outline 時，應先看這份 README，再參考既有範例，但不要機械複製內容；應根據當天主題調整最小骨架與教學順序。
